        ...
        if (htmlspecialcharscustom($this->input->post('submit'))) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($id == "") {
                    ...
                    // $item_info = $this->Common_model->getDataById($_POST['item_id'], "tbl_items");
                    // $purchase_info['item_name'] = $item_info['name'];
                    // $purchase_info['code'] = $item_info['code'];
            ...
            if ($id == "") {
                ...
                $data['paymentMethods'] = $this->Common_model->getAllPaymentMethod();
                $data['suppliers'] = $this->Common_model->getAllSupplierNameMobile();
                $data['items'] = $this->Common_model->getItemWithVariationAndOutetIdForDrowdown();
                $data['items1'] = $this->Common_model->getItemWithVariationAndOutetIdForDrowdown();
                $data['main_content'] = $this->load->view('purchase/addPurchase', $data, TRUE);
                $this->load->view('userHome', $data);
            } else {
                $data = array();
