...
class Ajax extends Cl_Controller
{
    ...
    public function saveItemImage()
    {
        ...
        if (createDirectory('uploads/items')) {
            $localImagePath = 'uploads/items/' . $imageName;
            file_put_contents($localImagePath, $data);

            $postData = json_encode([
                'image' => 'data:image/png;base64,' . base64_encode(file_get_contents($localImagePath)),
                'image_name' => $imageName
            ]);

            $nodeUrl = 'http://localhost:5000/upload';
            ...
            if (isset($responseData['image_name'])) {
                ...
                echo "Failed to upload the image to Node.js server.";
            }
        } else {
